...
/**
 * 
 * Class to compute average speeds for vehicles 
 * (for one minute in one segment)
 *
 */
public class AvgVehicleSpeeds{
	 ...
	 */
	private Map<Integer, Integer> avgsPerVehicle = new HashMap<Integer, Integer>();
	/**
	...
	public synchronized void addVehicleSpeed(int vehicleId, int vehicleSpeed) {
		...
		if (avgsPerVehicle.containsKey(vehicleId)) {
			int prevVehicleSpeed = ((Integer) avgsPerVehicle.get(vehicleId)).intValue();
