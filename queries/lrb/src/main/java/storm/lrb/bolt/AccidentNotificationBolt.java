...
public class AccidentNotificationBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		if(tuple.getSourceStreamId().equals(TopologyControl.POS_REPORTS_STREAM_ID)) {
			...
			SegmentIdentifier previousSegment = this.allCars
				.put(pos.getVehicleIdentifier(), pos.getSegmentIdentifier());
			// check whether there's an accident on the 4 upcoming segments
			for(int i = 0; i <= 4; i++) {
				int segmentToCheck = pos.getSegmentIdentifier().getSegment() + i;
				LOG.trace("checking segment '{}' for accidents", segmentToCheck);
				SegmentIdentifier accidentSegment = new SegmentIdentifier(pos.getSegmentIdentifier().getxWay(),
					segmentToCheck, pos.getSegmentIdentifier().getDirection());
	...
	private void updateAccidents(Tuple tuple) {
		...
		SegmentIdentifier accidentIdentifier = accident.getPosReport().getSegmentIdentifier();
	...
	private void sendAccidentAlert(PosReport pos, SegmentIdentifier previousSegment, AccidentImmutable accident) {
		...
		if(!previousSegment.equals(pos.getSegmentIdentifier())) {
