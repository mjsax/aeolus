		...
		if(lastSpeeds != null) {
			synchronized(lastSpeeds) {
				this.collector.emit(
					...
					new Values(segmentIdentifier.getxWay(), segmentIdentifier.getSegment(), segmentIdentifier
						.getDirection(), lastSpeeds.vehicleCount(), lastSpeeds.speedAverage(), minute));
				this.avgSpeedsMap.put(segmentIdentifier, new AvgVehicleSpeeds());
			}
		}
	}
