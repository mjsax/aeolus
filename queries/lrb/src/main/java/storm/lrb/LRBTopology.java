...
/**
 * An encapsulation of parameters which allows fast switches of the spout. Does 
 * everything essential for the cluster and allow to manipulate {@link Config} 
 * and {@link StormTopology} at will after creation in constructor.
 * 
 * @author richter
 */
public class LRBTopology {
    ...
    public LRBTopology(String nameext, List<String> fields, int xways, 
            int workers, int tasks, int executors, int offset, IRichSpout spout, 
            StopWatch stormTimer, boolean local, String histFile, String topologyNamePrefix) {
                TopologyBuilder builder = new TopologyBuilder();

        builder.setSpout(TopologyControl.START_SPOUT_NAME,
                spout,
                1);

        builder.setBolt(TopologyControl.SPLIT_STREAM_BOLT_NAME,
                new DispatcherBolt(),
                2 * xways)
                .setNumTasks(4 * xways)
                .shuffleGrouping(TopologyControl.START_SPOUT_NAME,
                        TopologyControl.SPOUT_STREAM_ID
                );//.allGrouping("Spout", "stormtimer");

        builder.setBolt(TopologyControl.AVERAGE_SPEED_BOLT_NAME,
                new AverageSpeedBolt(xways),
                xways * 3)
        ...
        builder.setBolt(TopologyControl.LAST_AVERAGE_SPEED_BOLT_NAME,
                new LastAverageSpeedBolt(),
                xways * 3)
                .fieldsGrouping(TopologyControl.AVERAGE_SPEED_BOLT_NAME,
        ...
        builder.setBolt(TopologyControl.TOLL_NOTIFICATION_BOLT_NAME,
                new TollNotificationBolt(stormTimer,
        ...
        builder.setBolt(TopologyControl.TOLL_FILE_WRITER_BOLT_NAME,
                new FileWriterBolt(topologyNamePrefix + "_toll", xways * 8, local),
                1)
                .allGrouping(TopologyControl.TOLL_NOTIFICATION_BOLT_NAME,
                        TopologyControl.TOLL_NOTIFICATION_STREAM_ID
                );

        builder.setBolt(TopologyControl.ACCIDENT_DETECTION_BOLT_NAME,
                new AccidentDetectionBolt(0),
                xways)
                .fieldsGrouping(TopologyControl.SPLIT_STREAM_BOLT_NAME,
                        TopologyControl.POS_REPORTS_STREAM_ID,
                        new Fields(TopologyControl.XWAY_FIELD_NAME,
        ...
        builder.setBolt(TopologyControl.ACCIDENT_FILE_WRITER_BOLT_NAME,
                new FileWriterBolt(topologyNamePrefix + "_acc", xways * 4, local),
                1)
                .allGrouping(TopologyControl.ACCIDENT_DETECTION_BOLT_NAME);

        builder.setBolt(TopologyControl.ACCOUNT_BALANCE_BOLT_NAME,
                new AccountBalanceBolt(),
                xways)
                .fieldsGrouping(TopologyControl.SPLIT_STREAM_BOLT_NAME,
                        TopologyControl.ACCOUNT_BALANCE_REQUESTS_STREAM_ID,
                        new Fields(TopologyControl.VEHICLE_ID_FIELD_NAME))
                .fieldsGrouping(TopologyControl.TOLL_NOTIFICATION_BOLT_NAME,
                        TopologyControl.TOLL_ASSESSMENT_STREAM_ID,
                        new Fields(TopologyControl.VEHICLE_ID_FIELD_NAME));

        builder.setBolt(TopologyControl.ACCOUNT_BALANCE_FILE_WRITER_BOLT_NAME,
                new FileWriterBolt(topologyNamePrefix + "_bal", xways * 4, local),
                1)
                .allGrouping(TopologyControl.ACCOUNT_BALANCE_BOLT_NAME);

        builder.setBolt(TopologyControl.DAILY_EXPEDITURE_BOLT_NAME,
                new DailyExpenditureBolt(histFile),
                xways * 1)
        ...
        builder.setBolt(TopologyControl.DAILY_EXPEDITURE_FILE_WRITER_BOLT_NAME,
                new FileWriterBolt(topologyNamePrefix + "_exp", xways * 2, local),
                1)
        ...
        stormConfig.registerSerialization(storm.lrb.model.AccBalRequest.class);
        stormConfig.registerSerialization(storm.lrb.model.DaiExpRequest.class);
        stormConfig.registerSerialization(storm.lrb.model.TTEstRequest.class);
        ...
        LOGGER.info(String.format("successfully created storm topology '%s'", 
                TopologyControl.TOPOLOGY_NAME));
    }
