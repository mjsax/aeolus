...
/**
 * This LRBtopology seperates processing of position reports for each
 * expressway, meaning there is a dedicated tollNotificationbolt, SegmentStatsbolt,
 * AccidentDetectionbolt, etc for each xway with its own partitioning
 * (though the degree of parallelism is the same for each boltclass).
 * 
 * The input stream is splitted in the dispatcherbolt which emits the position
 * reports to a position report stream of the given xway.
 * 
 * When submiting this topology the parameters -e and -t correspond to the
 * number of executors / tasks to use for the tollnotificationbolt per
 * expressway.
 * 
 */
public class LRBtopologyNaiveOptimized {
	...
	public static void main(String[] args) throws Exception {
		...
		int tasks = cmd.executors*cmd.tasks;
		
		String _lr = "lrbNaiveOpt_"+cmd.nameext+"_L"+cmd.xways+"_"+cmd.workers+"W_T"+tasks+"_"+cmd.executors+"E_O"+cmd.offset;
		...
		builder.setBolt("SplitStreamBolt", new DispatcherSplitBolt(cmd.xways), cmd.xways)
			.setNumTasks(tasks)
			.shuffleGrouping("Spout","stream");
		
		stormTimer.start(offset);
		System.out.println("using timer: " + stormTimer);
		...
		for (int faktor = 0; faktor < cmd.xways; faktor++) {
			...
			builder.setBolt(out, new TollNotificationBolt(stormTimer, faktor),cmd.executors).setNumTasks(tasks)
					.allGrouping(in2).allGrouping(in3)
					.fieldsGrouping("SplitStreamBolt", in, new Fields("vid"));
		...
		for (int faktor = 0; faktor < cmd.xways; faktor++) {
			...
			String instream = "TollNotification_" + faktor;
		...
		buildtemp = builder
				.setBolt("accountbalance", new AccountBalanceBolt(), cmd.xways * 2)
		...
		builder.setBolt("balfilewriter",
				new FileWriterBolt(_lr+"_bal", cmd.xways*2, cmd.submit), 1)
				.allGrouping("accountbalance");

		builder.setBolt("dailyexpenditure", new DailyExpenditureBolt(histfile),
				cmd.xways * 1)
				.shuffleGrouping("SplitStreamBolt", "DaiExpRequests");
		builder.setBolt("expfilewriter",
				new FileWriterBolt(_lr+"_exp", cmd.xways*1, cmd.submit), 1)
